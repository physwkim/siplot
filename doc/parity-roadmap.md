...
| ◐ | H | M | Mask save to file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | ALL five formats: `write_npy` (2D `uint8` `.npy`), `write_edf` (fabio-style EDF), `write_tiff` (grayscale `uint8` TIFF), `save_h5` (HDF5 dataset via pure-Rust `rust-hdf5`), `write_msk` (fit2d binary, `encode_mask_msk` byte-verified vs fabio) — each with `save_*`/`save_mask_*` wrappers. The file dialog is wired (`save_mask_dialog`, native rfd, npy/edf/tiff/hdf5/msk, extension-dispatched). HDF5 save is faithful to silx's "a" mode (`open_rw` append + delete-and-recreate the mask dataset, else create). Remaining ◐: HDF5 path defaults to `mask` (no interactive picker). Live row 200. |
