...
| ◐ Partial | M | L | Mask file save (npy/edf/tif/h5/msk) | MaskToolsWidget.py:104-141 | `.npy`, `.edf`, `.tif`/`.tiff`, AND `.h5` (HDF5) encode now ported: hand-written `encode_mask_edf` (fabio-style ASCII `{…}` header + raw `uint8` body, 512-aligned), `encode_mask_tiff` (single-page grayscale `uint8` via the `tiff` crate), and `write_mask_hdf5` (2D `uint8` dataset via the pure-Rust `rust-hdf5` crate) with `MaskToolsWidget::write_edf`/`write_tiff`/`save_h5` + `save_edf`/`save_tiff` + `save_mask_edf`/`save_mask_tiff`/`save_mask_h5` mirroring the npy path. `save_mask_dialog` opens a native rfd save picker (npy/edf/tiff/hdf5 filters) and dispatches by extension via the single-owner `resolve_mask_save_format` (missing→npy default, unsupported rejected — faithful to silx `save`). HDF5 divergence: writes a fresh standalone file (silx appends in "a" mode). Remaining: fit2d `.msk` encoder (no local fabio reference) |
| ◐ Partial | M | L | Mask file load (npy/edf/tif/h5/msk) | MaskToolsWidget.py:589-629 | `read_npy`, `read_edf`, `read_tiff`, AND `load_h5` decode+apply a 2D uint8 mask through the shared `apply_loaded_mask` crop/pad+commit owner (`decode_mask_edf` reads the fabio `{…}` header; `decode_mask_tiff` reads a single-channel `uint8` TIFF; `read_mask_hdf5`/`read_mask_hdf5_auto` read a 2D `uint8` dataset via `rust-hdf5`, `ndims()==2` guarded); `load_edf`/`load_tiff`/`load_h5` + `load_mask_edf`/`load_mask_tiff`/`load_mask_h5` mirror the npy wrappers. HDF5 dataset selection: `mask_datasets` enumerates 2D datasets and `load_h5_dataset` loads a chosen one (load auto-selects the single/first dataset); the interactive Qt picker UI is not ported. `load_mask_dialog` opens a native rfd open picker (npy/edf/tiff/hdf5) dispatching by extension via `resolve_mask_load_format` (unknown extension rejected, faithful to silx `load`). Missing: fit2d `.msk` decoder (no local fabio reference) |
...
| ◐ | H | M | Mask save to file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | `write_npy` writes a 2D `uint8` `.npy`; `write_edf` fabio-style EDF; `write_tiff` a grayscale `uint8` TIFF (`tiff` crate); `save_h5` a 2D `uint8` HDF5 dataset (`render::save::write_mask_hdf5`, pure-Rust `rust-hdf5`) — each with `save_*`/`save_mask_*` wrappers. The file dialog is wired (`save_mask_dialog`, native rfd, npy/edf/tiff/hdf5, extension-dispatched). HDF5 divergence: fresh standalone file (silx appends in "a" mode). Remaining ◐: fit2d `.msk` encoder (no local fabio reference). Live row 200. |
| ◐ | H | M | Mask load from file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:589-629 (load method), 630-677 (_loadMask dialog)` | `read_npy`, `read_edf`, `read_tiff`, and `load_h5` read a 2D `uint8` mask and apply it through the shared `apply_loaded_mask` owner (crop/pad to the image, committed to undo; silx `load` npy/edf/tif/h5 branches); `read_mask_hdf5` reads a 2D `uint8` dataset via `rust-hdf5` (`ndims()==2` guarded). Dataset selection: `mask_datasets` enumerates + `load_h5_dataset` loads a named dataset (load auto-selects the first); interactive Qt picker UI not ported. The file dialog is wired (`load_mask_dialog`, native rfd, npy/edf/tiff/hdf5, extension-dispatched). Remaining ◐: fit2d `.msk` decoder (no local fabio reference). |
