...
| ◐ Partial | M | L | Mask file save (npy/edf/tif/h5/msk) | MaskToolsWidget.py:104-141 | `.npy`, `.edf`, AND `.tif`/`.tiff` encode now ported: hand-written `encode_mask_edf` (fabio-style ASCII `{…}` header + raw `uint8` body, 512-aligned) and `encode_mask_tiff` (single-page grayscale `uint8` via the `tiff` crate) with `MaskToolsWidget::write_edf`/`write_tiff` + `save_edf`/`save_tiff` + `save_mask_edf`/`save_mask_tiff` mirroring the npy path. `save_mask_dialog` opens a native rfd save picker (npy/edf/tiff filters) and dispatches by extension via the single-owner `resolve_mask_save_format` (missing→npy default, unsupported rejected — faithful to silx `save`). Remaining: HDF5/msk encoders (`hdf5`/fabio crates) |
| ◐ Partial | M | L | Mask file load (npy/edf/tif/h5/msk) | MaskToolsWidget.py:589-629 | `read_npy`, `read_edf`, AND `read_tiff` decode+apply a 2D uint8 mask through the shared `apply_loaded_mask` crop/pad+commit owner (`decode_mask_edf` reads the fabio `{…}` header; `decode_mask_tiff` reads a single-channel `uint8` TIFF via the `tiff` crate, rejecting multi-channel/non-uint8); `load_edf`/`load_tiff` + `load_mask_edf`/`load_mask_tiff` mirror the npy wrappers. `load_mask_dialog` opens a native rfd open picker (npy/edf/tiff) dispatching by extension via `resolve_mask_load_format` (unknown extension rejected, faithful to silx `load`). Missing: HDF5/msk decoders (external-crate-bound), HDF5 dataset selection |
...
| ◐ | H | M | Mask save to file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | `write_npy` writes a 2D `uint8` `.npy`; `write_edf` fabio-style EDF; `write_tiff` a grayscale `uint8` TIFF (`render::save::encode_mask_tiff`, `tiff` crate) — each with `save_*`/`save_mask_*` wrappers. The file dialog is wired (`save_mask_dialog`, native rfd, npy/edf/tiff, extension-dispatched). Remaining ◐: HDF5/msk encoders (external-crate-bound). Live row 200. |
| ◐ | H | M | Mask load from file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:589-629 (load method), 630-677 (_loadMask dialog)` | `read_npy`, `read_edf`, and `read_tiff` read a 2D `uint8` mask and apply it through the shared `apply_loaded_mask` owner (crop/pad to the image, committed to undo; silx `load` npy/edf/tif branches); `decode_mask_tiff` reads a single-channel `uint8` TIFF (`tiff` crate). The file dialog is wired (`load_mask_dialog`, native rfd, npy/edf/tiff, extension-dispatched). Remaining ◐: HDF5/msk decoders (external-crate-bound). |
