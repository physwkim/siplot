...
| ◐ Partial | L | L | SaveAction PDF/EPS/JPEG | PlotWidget.py:3232-3242 | EPS now exported: `render::save::encode_eps` embeds the raster via a PostScript `colorimage` (Y-flip matrix, ASCII-hex body) — the SVG-style faithful substitute for silx's matplotlib vector EPS, no new dep. `SaveFormat::Eps` + `from_extension("eps")` + dispatch wired. PNG/PPM/SVG/TIFF/EPS supported. Remaining: PDF (single-image XObject, next); JPEG needs a DCT encoder crate (deferred under the no-new-dep pattern) |
