...
| ◐ | H | L | ROI statistics (min/max/mean/sum/integral, raw/net counts, COM/coords) | `CurvesROIWidget.py:355-430, ROIStatsWidget.py (full file)` | `roi_stats` computes count/min/max/mean/sum/integral per ROI (image + curve); `curve_roi_counts` adds curve raw/net counts + raw/net area (silx `CurvesROIWidget.computeRawAndNetCounts/Area`). The original "missing peak detection" note was a misattribution — "peaks" is NOT a silx ROI stat: neither `CurvesROIWidget` (columns from/to/type/rawcounts/netcounts/rawArea/netArea) nor the `stats` framework (`StatMin/Max/Delta/CoordMin/CoordMax/COM`) computes peaks; peak-search lives in the fit cluster (`peak_search`, already ported). Real residual: per-ROI COM/coordMin/coordMax — the math exists and is tested in `core::Stats` (`com`/`coord_min`/`coord_max`) and is available per CURVE ROI via `Stats::for_curve_roi`, but the widget-level `RoiStats` (`image_roi_stats`/`curve_roi_stats`) does not yet surface them, and `Stats::for_image` only windows rectangularly (not arbitrary `Roi::contains` shapes). |
